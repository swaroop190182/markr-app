...
const STAGES     = ['Idea', 'Early', 'Launch', 'Growth']
...
export default function EditAppModal({ appId, onClose }: { appId: number; onClose: () => void }) {
  ...
  const setStep = (i: number, s: Step) => setSteps(prev => prev.map((v,idx) => idx===i ? s : v))

  const hasCreds = !!(testUser && (testPass || app.testCreds?.password))

  function saveOnly() {
    updateApp(appId, {
      name, url, platform: platform as any, stage: stage as any, category, desc,
      testCreds: testUser ? {
        user: testUser,
...
BRAND_VOICE: [3-4 sentences on voice]`,
      ...
      const gf = (k: string) => (raw.match(new RegExp(k+':\\s*([^\\n]+)'))??[])[1]?.trim()??''
      ...
      if (hasCreds && effectivePass) {
        ...
        try {
          productTest = await runProductTest({
            ...app, name, url, desc: newDesc, features,
            testCreds: { user: testUser, password: effectivePass, loginUrl: loginUrl||url, flows: testFlows }
          }, effectivePass)
        } catch (e) {
      ...
      updateApp(appId, {
        name, url, platform: platform as any, stage: stage as any, category,
        desc: newDesc, brand, pillars, features, audience, problem, diff,
        testCreds: { user: testUser, password: effectivePass, loginUrl: loginUrl||url, flows: testFlows },
        productTest
      })
      onClose()
      toast(hasCreds && productTest && !productTest.error
        ? `✅ ${name} updated & re-tested! Score: ${productTest.overall_score}/100`
        : `✅ ${name} updated!`, 4000)
  ...
  return (
    <Modal title="Edit App" subtitle="Update details, credentials, or re-run the product test." onClose={onClose}>
      ...
      <div style={{ border:'1px solid rgba(124,111,247,.3)', borderRadius:'var(--r)', overflow:'hidden', marginBottom:4 }}>
        <div onClick={()=>setCredsOpen(!credsOpen)}
